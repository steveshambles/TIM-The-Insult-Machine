"""Tim V1.91-WL Win-lin version
By Steve Shambles June 2019-20
Updated June 2020.
...
Files required in same folder as TIM-1-90-WL.py:
tim-logov3-360x195
tim-help.txt
...
V1.91-WL log:
Improved code with in the accents menu ticks, it was a horrific mess.

V1.90-WL log:
Many small code improvements, need more though.
some tiny cosmetic improvements
changed height of gui as was too tall on screen.
added chinese accent.
some extra idiot checks,
Updated and improved help text.
linted through pylint and pycodestyle,

To do:
Need to fix bare excepts, just don't know exactly how yet.
Improve horrific code starting at line 489, don't know how.
Improve text alignments in linux pop up boxes, yuk. 
"""
import os
import random
import time
...
root = Tk()
root.title('TIM-The Insult Machine V1.91-WL')


def about_menu():
    """About program."""
    messagebox.showinfo('Nosey Aint ya', 'The Insult Machine V1.91-WL\n\n'
...
def help_text():
    ...
    if not os.path.isfile('tim-help.txt'):
        messagebox.showinfo('You mad pidgeon botherer!',
...
def generate_insult():
    ...
    with open('saved_insults.txt', 'a') as f:
...
def get_insult():
    """Get insult string randomly from word lists
       and store in Glo.full_insult."""
    ...
    try:
        ...
        messagebox.showerror('You massive head of a dick!',
                             ...
                             'this freaking mess you freak girafffe.')
        root.destroy()
        root.quit()

    # Create the actual insult text by joing the random words together.
    # Example:"You foul smelling pile of stinking helmet scabs".
    Glo.full_insult = 'You ' + WORD_1 + ' ' + WORD_2 + ' of ' +  \
...
def generate_mp3():
    ...
    try:
        ...
        messagebox.showerror('Oh arescrackz dude!',
                             ...
                             'with no sound, or come back later.')
    ...
    # No errors so let's do it:
    # If 'hear insult' is checked, then play insult
    if VAR1.get() == 1:
        # Plays the mp3 using the default app on your system.

        # Had to do this way otherwise would not work on Windows, weird.
        if sys.platform.startswith('linux'):
            play_mp3 = 'saved_mp3s/'+file_name

        else:
            # Why need 2 back slashes here,but 1fwd slash other places?
            play_mp3 = 'saved_mp3s\\'+file_name
    ...
    if VAR2.get() == 0:
        time.sleep(3)  # Give mp3 time to stop playing, just in case.
        if os.path.isfile('saved_mp3s/'+file_name):
            os.remove('saved_mp3s/'+file_name)
...
def hear_sel_insult():
    ...
    # If no line properly selected.
    if main_lst_bx.curselection() is ():
        return
    ...
    # Delete the mp3 if "save mp3" is not ticked.
    if VAR2.get() == 0:
        time.sleep(3)
        if os.path.isfile('saved_mp3s/'+file_name):
            os.remove('saved_mp3s/'+file_name)
...
def save_fav():
    ...
    try:
        ...
        # Append insult to text file for storage.
        with open('my_fav_insults.txt', 'a') as f:
            f.write(cop + '\n')
...
def use_clean_insults():
    ...
    messagebox.showinfo('For the more sensitive grown ups among us!',
                        'Clean insults selected.\n\n'
                        'All swear words and most other offensive words\n'
                        'have been removed, Are you happy now?')
    clear_but()
...
def use_rude_insults():
    """Called when selected from drop-down menu."""
    # If already using rude insults, abort.
    ...
    messagebox.showwarning('Ohh, scary darey!',
                           'Rude insults selected.\n\n'
                           'Warning:\n'
                           'Every sick, foul and disgusting word I can think\n'
                           'of will be used. You have been warned.\n\n')
    clear_but()
...
def visit_github():
    """Open TIMs GitHub page."""
    webbrowser.open('https://github.com/steveshambles/TIM-The-Insult-Machine')
...
FILE_MENU.add_command(label='TIM Python source code',
                      command=visit_github)
FILE_MENU.add_separator()
FILE_MENU.add_command(label='Make small donation via PayPal',
                      command=donate_me)
FILE_MENU.add_separator()
FILE_MENU.add_command(label='Google Translate',
                      command=google_trans)
...
accents = [ukVar, chVar, auVar, usVar, caVar, deVar, frVar,
               itVar, hiVar, jaVar, ruVar, esVar]
...
INSULTS_MENU.add_checkbutton(label='Chinese PRC',
                             variable=chVar,
                             command=lambda: change_lang('zh-CN'))
...
# Capture window exit on clicking X.
root.protocol("WM_DELETE_WINDOW", quit_tim)
